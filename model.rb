...
module FBHot
  ...
  class User
    ...
    def sex 
      @sex ||= FBHot.redis.hget "user:#{@prefix_id}", "#{@suffix_id}:sex"
    end
    
    def pic_square 
      @pic_square ||= FBHot.redis.hget "user:#{@prefix_id}", "#{@suffix_id}:pic_square" 
    end

    def pic_big 
      @pic_big ||= FBHot.redis.hget "user:#{@prefix_id}", "#{@suffix_id}:pic_big" 
    end
    
    def name 
      @name ||= FBHot.redis.hget("user:#{@prefix_id}", "#{@suffix_id}:name").force_encoding("utf-8") 
    end
    ...
    def win_count 
      @win_count ||= FBHot.redis.hget("user:#{@prefix_id}", "#{@suffix_id}:win_count") 
      @win_count ||= 0
    end

    def lose_count 
      @lose_count ||= FBHot.redis.hget "user:#{@prefix_id}", "#{@suffix_id}:lose_count"
      @lose_count ||= 0
    end
